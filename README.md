...
Actualmente, Funkomania Backend `0.5.0-SNAPSHOT-1` incluye las siguientes funcionalidades:

- API REST con endpoints para registrar usuarios, inicio de sesión, cierre de sesión de un usuario, obtener el catálogo de productos, ofertas, categorías, gestionar el perfil de usuario y direcciones.
...
1. Descargar el proyecto abriendo un terminal y clonar el repositorio usando Git en un directorio:
   ```bash
   git clone https://github.com/JuanAlberticoHF/funkomania-backend.git --branch feature/gestion-perfil-usuario
   ```
   - Esto descargará el proyecto en tu máquina local de la rama `feature/gestion-perfil-usuario`, que es la rama de desarrollo actual.
